...
Note: Syarat dan Ketentuan mungkin berlaku`.trim(), wm, 'Menu', '#menu', 'Owner', '#owner', m)
    ...
    if (new Date() * 1 - setting.status > 1000) {
        ...
        await this.setStatus(`Im DanzBot 🤖 || ⏰ Aktif selama ${uptime} jam |(*delay*🌕)| 👥 User : ${Object.keys(global.db.data.users).length} User || 🎐 Mode: ${global.opts['self'] ? 'Private' : setting.groupOnly ? 'Hanya Grup' : 'Publik by.rozi'}`).catch(_ => _)
